...
function fish_right_prompt --description 'Write out the right-hand prompt'
	...
	if not test "0" -eq $__CURRENT_GIT_STATUS_PARAM_COUNT
        ...
        # printf ' %s%s%s %s %s%s %s' (set_color -b (yellow)) (date) (set_color (offwhite)) "|" (set_color 777) $_display_status (set_color normal)
        printf ' %s%s%s %s %s%s %s' "\x1b[100m\x1b[97m" (date) (set_color (offwhite)) "|" (set_color 777) $_display_status "\x1b[m"
        #printf "\x1b[100m\x1b[97m<\x1b[m"
    end
end
